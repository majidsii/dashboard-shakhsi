#!/usr/bin/env python3
from pathlib import Path

root = Path(__file__).resolve().parents[1]

checks = {
    'lib/core/notifications/notification_coordinator.dart': [
        'final class NotificationCoordinator',
        'await _repository.upsert(request);',
        'await _scheduler.schedule(request);',
        'Future<void> replaceAll',
        'Future<void> reconcileFromPersistence',
        '_deduplicate',
    ],
    'lib/core/notifications/noop_notification_scheduler.dart': [
        'final class NoopNotificationScheduler',
        'implements NotificationScheduler',
    ],
    'lib/core/providers/persistence_providers.dart': [
        'notificationSchedulerProvider',
        'notificationCoordinatorProvider',
        'NoopNotificationScheduler',
    ],
}

for relative, markers in checks.items():
    path = root / relative
    if not path.is_file():
        raise SystemExit(f'Missing GREEN file: {relative}')
    source = path.read_text(encoding='utf-8')
    for marker in markers:
        if marker not in source:
            raise SystemExit(f'Missing GREEN marker {marker!r} in {relative}')

print('Phase 1 Task 3 GREEN contract verified.')
