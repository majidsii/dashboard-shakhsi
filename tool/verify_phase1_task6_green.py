#!/usr/bin/env python3
from pathlib import Path

root = Path(__file__).resolve().parents[1]

checks = {
    'lib/core/notifications/native_notification_gateway.dart': [
        'abstract interface class NativeNotificationGateway',
        'Future<void> showNow',
        'Future<List<NativePendingNotification>> pending',
    ],
    'lib/core/notifications/platform_notification_scheduler.dart': [
        'final class PlatformNotificationScheduler',
        'NotificationDeliveryPolicy.contentFor',
        'NotificationPayloadCodec.encode',
        'StableNotificationId.fromScheduleId',
        'supportsScheduledDelivery',
        'Future<void> reconcile',
    ],
    'lib/core/notifications/flutter_local_notifications_driver.dart': [
        'implements LocalNotificationsDriver, NativeNotificationGateway',
        '_plugin.zonedSchedule',
        'AndroidScheduleMode.inexactAllowWhileIdle',
        '_plugin.pendingNotificationRequests',
        "const NotificationDetails",
    ],
}

for relative, markers in checks.items():
    path = root / relative
    if not path.is_file():
        raise SystemExit(f'Missing GREEN file: {relative}')
    source = path.read_text(encoding='utf-8')
    for marker in markers:
        if marker not in source:
            raise SystemExit(f'Missing GREEN marker {marker!r} in {relative}')

print('Phase 1 Task 6 GREEN contract verified.')
